...
for bat_file in glob.glob('RibbonBridge_Dist/*.bat'):
    with open(bat_file, 'w', encoding='cp949', errors='ignore') as f:
        f.write(bat_content)

print("Batch file encoding rewritten to CP949 successfully.")

# Create the zip directly ensuring RibbonBridge_Dist contents are archived properly
archive_name = 'RibbonBridge_15_7_Final'
shutil.make_archive(archive_name, 'zip', 'RibbonBridge_Dist')
print(f"Created {archive_name}.zip successfully.")
