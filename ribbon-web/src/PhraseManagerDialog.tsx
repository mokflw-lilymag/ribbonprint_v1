      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col font-sans h-[600px]">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-6">
          ...
          <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700 space-y-3">
            <h3 className="text-xs font-bold text-slate-300">새로운 상용구 추가</h3>
            <div className="flex flex-col gap-3">
              ...
              <button disabled={isLoading} onClick={handeAdd} className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-2 rounded-lg flex items-center justify-center gap-2 transition-colors">
          ...
          <div className="space-y-4">
            <h3 className="text-xs font-bold text-slate-300 border-b border-slate-700 pb-2">등록된 상용구 목록</h3>
            {categories.map(cat => (
              <div key={cat} className="space-y-2">
                <h4 className="text-blue-400 text-sm font-bold bg-slate-800/80 px-3 py-1.5 rounded">{cat}</h4>
                  ...
                  {phrases.filter(p => p.category === cat).map(p => (
                    <div key={p.id} className="flex items-center justify-between bg-slate-800 border border-slate-700 px-3 py-2 rounded-lg hover:border-slate-500 group">
                      <div className="flex flex-col">
                        <span className="text-sm font-bold text-white leading-tight">{p.text}</span>
